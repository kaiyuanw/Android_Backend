...
class CreateStreamHandler(webapp2.RequestHandler):
    def post(self):
        ...
        if str(self.request.get("streamName")) == '':
            ...
            stream = Stream(streamName=self.request.get("streamName"), creator=users.get_current_user().user_id(),
                            photos=[], subscribers=[], views=0, viewsPastHour=0,
                            coverURL=self.request.get("coverURL"), optionalMessage=self.request.get("message"),
                            tags=self.request.get("tags"), inviteSubscribers=self.request.get("subscribers"))
...
class SubscriptionPage(webapp2.RequestHandler):
    def get(self, url):
        ...
        if user is None:
            ...
            return
        self.response.out.write(MAIN_PAGE_TEMPLATE_HEAD % (
            users.create_logout_url('/'), '/manage', '/create', '/viewAllStreams', '/search', '/trending'))
        self.response.out.write(
            '<br><h3>Are you sure you want to subscribe?</h3><br><form action="/subscriptionHandler/%s" method="POST"><input type="submit" value="YES!"></form>' % url)
        ...
        if userInputString == "":
            ...
            for stream in stream_query:
                ...
                for keyword in userInput:
                    if str(keyword.lower()) in (tagsArray2) or str(keyword.lower()) in (
                            streamNameArray2) and stream not in streamsFound:
    ...
    def write_blob(self, s, data, info):
        ...
        for stream in q:
            if str(stream.key.id()) == s:
                ...
                picloc = ndb.GeoPt(-57.32652122521709 + 114.65304245043419 * random.random(),
                                   -123.046875 + 246.09375 * random.random())
...
class AndViewAllStreams(webapp2.RequestHandler):
    def get(self):
        ...
        dictPassed = {}
        if user is None:
            dictPassed = {'streamDictsArr': streamDictsArr, 'user': None,
                          'logoutURL': users.create_logout_url('/andmain')}
        else:
            dictPassed = {'streamDictsArr': streamDictsArr, 'user': str(user.user_id()),
                          'logoutURL': users.create_logout_url('/andmain')}
...
class AndViewNearbyPhotos(webapp2.RequestHandler):
    def get(self, photoIndexes, currentLocation):
        ...
        dictPassed = {'user': None, 'morePhotos': morePhotos, 'logoutURL': users.create_logout_url('/andmain'),
                      'indexURL': indexURL, 'allPhotos': passedPhotos}  # 'displayPhotoList' : displayStreamList
        jsonObj = json.dumps(dictPassed, sort_keys=True, indent=4, separators=(',', ': '))
        self.response.write(jsonObj)
